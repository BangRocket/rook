...
impl Neo4jGraphStore {
    ...
    pub async fn new(config: GraphStoreConfig) -> RookResult<Self> {
        ...
        let username = config.username.clone().unwrap_or_else(|| "neo4j".to_string());
        let password = config.password.clone().unwrap_or_default();

        let graph = Graph::new(&uri, &username, &password)
            .await
            .map_err(|e| RookError::graph_store(format!("Failed to connect to Neo4j: {}", e)))?;
...
impl GraphStore for Neo4jGraphStore {
    async fn add(
        ...
        // This is a simplified implementation - in production, you'd use an LLM to extract entities
        for message in messages {
            let content = &message.content;

            // Create a node for the message content
    ...
    async fn search(
        ...
        let mut result = self
            ...
            .execute(q)
            .await
            .map_err(|e| RookError::graph_store(format!("Failed to search graph: {}", e)))?;
    ...
    async fn delete_all(&self, filters: &GraphFilters) -> RookResult<()> {
        ...
        self.graph
            .run(q)
            .await
            .map_err(|e| RookError::graph_store(format!("Failed to delete from graph: {}", e)))?;
    ...
    async fn get_all(&self, filters: &GraphFilters) -> RookResult<Vec<Entity>> {
        ...
        let q = query(
            r#"
            MATCH (u:User {id: $user_id})-[r]->(m)
            RETURN labels(m)[0] as entity_type, m.content as name, properties(m) as properties
            "#,
        )
        .param("user_id", user_id);
        ...
        while let Some(row) = result
            ...
            .map_err(|e| RookError::graph_store(format!("Failed to fetch row: {}", e)))?
        {
            let entity_type: String = row.get("entity_type").unwrap_or("Memory".to_string());
