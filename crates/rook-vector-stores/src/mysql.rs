//! MySQL vector store implementation (Azure MySQL with vector support).
...
impl MySQLVectorStore {
    /// Create a new MySQL vector store.
    pub async fn new(config: VectorStoreConfig) -> RookResult<Self> {
        let url = config
            .url
            .clone()
            .ok_or_else(|| RookError::Configuration("MySQL connection string required".to_string()))?;

        let pool = MySqlPoolOptions::new()
            .max_connections(5)
            .connect(&url)
            .await
            .map_err(|e| RookError::vector_store(format!("Failed to connect to MySQL: {}", e)))?;

        Ok(Self { pool, config })
    }

    fn vector_to_string(vector: &[f32]) -> String {
        let values: Vec<String> = vector.iter().map(|f| f.to_string()).collect();
        format!("[{}]", values.join(","))
    }
...
impl VectorStore for MySQLVectorStore {
    async fn create_collection(
        ...
        // Create table with vector stored as JSON array
    ...
    async fn search(
        ...
        // MySQL doesn't have native vector similarity, so we compute it in application
        // For production, consider using MySQL HeatWave with vector support
...
impl MySQLVectorStore {
    fn build_filter(filter: &Filter) -> String {
        let conditions: Vec<String> = filter
            ...
            .iter()
            .filter_map(|cond| {
                let field = format!("JSON_UNQUOTE(JSON_EXTRACT(metadata, '$.{}'))", cond.field);
