...
use elasticsearch::{
    ...
    Elasticsearch, GetParts, IndexParts, SearchParts, DeleteParts, BulkParts,
};
use serde_json::json;
...
impl ElasticsearchVectorStore {
    /// Create a new Elasticsearch vector store.
    pub async fn new(config: VectorStoreConfig) -> RookResult<Self> {
        ...
        let transport = Transport::single_node(&url)
            .map_err(|e| RookError::vector_store(format!("Failed to create ES transport: {}", e)))?;
    ...
    fn distance_to_es(metric: &DistanceMetric) -> &'static str {
        match metric {
...
impl VectorStore for ElasticsearchVectorStore {
    async fn create_collection(
        ...
        let similarity = Self::distance_to_es(&metric);
    ...
    async fn search(
        ...
        if let Some(f) = filter {
            query["knn"]["filter"] = Self::build_filter(&f);
        }
