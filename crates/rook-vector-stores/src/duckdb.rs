//! DuckDB vector store implementation (stub).
//! Full implementation requires the duckdb crate with vector extension support.
...
impl VectorStore for DuckDBVectorStore {
    ...
    async fn update(&self, collection_name: &str, id: &str, vector: Option<Vec<f32>>, metadata: Option<HashMap<String, serde_json::Value>>) -> RookResult<()> {
        if let Some(collection) = self.storage.write().await.get_mut(collection_name) {
            if let Some(r) = collection.get_mut(id) {
                if let Some(v) = vector { r.vector = v; }
                if let Some(m) = metadata { r.metadata.extend(m); }
            }
        }
        Ok(())
    }
    ...
    async fn list(&self, collection_name: &str, _filter: Option<Filter>, limit: Option<usize>) -> RookResult<Vec<VectorRecord>> {
        let storage = self.storage.read().await;
        let mut records: Vec<_> = storage.get(collection_name).map(|c| c.values().cloned().collect()).unwrap_or_default();
